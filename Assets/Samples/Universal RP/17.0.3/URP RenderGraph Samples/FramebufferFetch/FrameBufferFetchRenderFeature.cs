    ...
    class FrameBufferFetchPass : ScriptableRenderPass
    {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            if (destinationDesc.msaaSamples == MSAASamples.None || RenderGraphUtils.CanAddCopyPassMSAA())
            {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<PassData>("Set Global Texture",
                           out var passData))
                {
                    builder.AllowPassCulling(false);
                    builder.SetRenderFunc((PassData data, RasterGraphContext context) => ExecutePass(data, context));
                    builder.SetGlobalTextureAfterPass(fbFetchDestination, Shader.PropertyToID("_InputTexture"));
                }
