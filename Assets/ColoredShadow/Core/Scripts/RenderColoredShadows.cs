...
namespace ColoredShadow.Core.Scripts
{
    ...
    {
        public bool renderingToSceneView;
        ...
        private Shader sceneDebugShader;
        ...
        public RenderColoredShadows(GraphicsBuffer lightInformationBuffer)
        {
            ...
            depthShader = Shader.Find("CustomShadows/ColorOnlyNoDepth");
            sceneDebugShader = Shader.Find("Shader Graphs/OverrideColShadow_DebugView");
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>("CAPTURE_COL_SHADOWS", out var passData, profilingSampler))
            {
                ...
                builder.SetRenderFunc((PassData data, RasterGraphContext rgContext) =>
                {
                    ...
                    foreach (ShadowPass shadowPass in data.shadowPasses)
                    {
                        rgContext.cmd.SetViewport(new Rect(shadowPass.texturePosX, shadowPass.texturePosY, shadowPass.textureWidth, shadowPass.textureHeight));
                        rgContext.cmd.SetViewProjectionMatrices(shadowPass.viewMatrix, shadowPass.projectionMatrix);
                        rgContext.cmd.DrawRendererList(shadowPass.rendererList);
                    }
                });
            }
            ...
            if (anyDepthMergePass)
            {
                using (var builder = renderGraph.AddRasterRenderPass<PassData>("GET_DEPTH_OF_SHADOW_RECEIVERS", out var passData, profilingSampler))
                {
                    ...
                    builder.SetRenderFunc((PassData data, RasterGraphContext rgContext) =>
                    {
                        rgContext.cmd.ClearRenderTarget(true, false, Color.blueViolet);
            ...
            if (renderingToSceneView)
            {
                TextureDesc destinationDescScene = renderGraph.GetTextureDesc(resourceData.cameraColor);
                destinationDescColor.format = GraphicsFormat.R16_SFloat;
                destinationDescColor.filterMode = FilterMode.Bilinear;
                destinationDescColor.name = "SOURCE_SCENE";
                destinationDescColor.width = cameraData.cameraTargetDescriptor.width;
                destinationDescColor.height = cameraData.cameraTargetDescriptor.height + 1;
                destinationDescColor.enableRandomWrite = true;
                // destinationDescColor.clearBuffer = true;
                TextureHandle destinationScene = renderGraph.CreateTexture(destinationDescScene);
                ...
                {
                    Debug.LogError($"Couldnt get ScriptableCullingParameters from scene view");
                }
                
                CullingResults cullingResults = cullContextData.Cull(ref scriptableCullingParameters);
                RendererListHandle rendererList = InitRendererLists(renderingData, lightData, renderGraph, cullingResults, cameraData, sceneDebugShader);
                
                using (var builder = renderGraph.AddRasterRenderPass<PassDataScene>("CAPTURE_DEBUG_SCENE_VIEW", out var passData, profilingSampler))
                {
                    builder.SetRenderAttachment(destinationScene, 0);
                    builder.SetRenderAttachmentDepth(destinationDepth);
                    ...
                    builder.SetRenderFunc((PassDataScene data, RasterGraphContext rgContext) =>
                    {
        ...
        public static LightInformation GetLightInformation(CustomLight light)
        {
